...
namespace BotLibary.Bots.Masters
{
    ...
    public class MasterBot : AbstractMasterBot, IBot
    {
        ...
        public MasterBot(BotOptions<MasterBotConfig, PersonalMasterBotConfig> options)
        {
            ...
            context = new DataBaseMastersConnector(new UpdaterEntity(botConfig.DataBaseName), new CreaterEntity(botConfig.DataBaseName), new EntityFinder(botConfig.DataBaseName));
            ...
            OnMessageHandler = new OnMessage(context, options, bot, AdminLog, Log, dateFunction);
        }
            ...
            if (e.Message != null && (e.Message.Type != Telegram.Bot.Types.Enums.MessageType.Text || !string.IsNullOrEmpty(e.Message.Text)))
            {
               ...
               // ConsoleMessage?.Invoke(currentUser == null ? $"\nПользователь с chatId {e.Message.Chat.Id} не найден\n" : $"\nПользователь с chatId {e.Message.Chat.Id} уже заходил\n");
                if (currentUser == null)
                {
                    ...
                    currentUser = await context.Creater.CreateNewUserAsync(e.Message.From.Username, e.Message.From.FirstName, e.Message.From.LastName, e.Message.Chat.Id);
                    return;
                }
                OnMessageHandler.UserOnMessage(sender, e);
                if (currentUser.IsAdmin)
                {
                    if (admin.Status != null)
                    {
                        if (admin.Status.Split('/')[0] == "AddApp")
                        {
                            ...
                            {
                                admin.Status = "";
                                await context.Updater.UpdateUserAsync(admin);
                                Appointment app = await context.Finder.FindAppointmentAsync(appId);
                                if (e.Message.Text.Length == 5)
                                {
                                    app.AppointmentTime = e.Message.Text;
                                    await context.Updater.UpdateAppAsync(app);
                                ...
                                {
                                    await context.Updater.DeleteAppAsync(app);
                                    app = null;
                    ...
                    {
                        List<Month> month = await context.Finder.GetMonthsAsync();
                    ...
                    {
                        List<Month> months = await context.Finder.GetMonthsAsync();
                    ...
                    {
                        List<User> users = await context.Finder.FindUsersAsync();
                    ...
                    {
                        List<Appointment> apps = await context.Finder.FindConfirmAppointmentsAsync(false);
                        ...
                        {
                            User user = await context.Finder.FindUserAsync(app.User);
                            Day day = await context.Finder.FindDayAsync(app.Day);
                            await bot.SendTextMessageAsync(admin.ChatId, $"Запись на {day.Date}.{day.MonthNumber} на время {app.AppointmentTime}\n" +
                                $"Записался {user}", replyMarkup: AdminKeyboard.GetConfirmKeyboard(app.AppointmentId, options, Codes.AdminConfirm, user.UserId));
                        }
                        return;
                    }
                    if (e.Message.Text == personalConfig.AdminButtons["LOOKCONFIRM"])
                    {
                        List<Appointment> apps = await context.Finder.FindConfirmAppointmentsAsync(true);
                        if (apps == null || apps.Count == 0)
                        {
                        ...
                        foreach (Appointment app in apps)
                        {
                            DataBase.Models.User user = await context.Finder.FindUserAsync(app.User);
                            Day day = await context.Finder.FindDayAsync(app.Day);
                            await bot.SendTextMessageAsync(admin.ChatId, $"Запись на {day.Date}.{day.MonthNumber} на время {app.AppointmentTime}\n" +
                                $"Записался {user}", replyMarkup: AdminKeyboard.GetCanccelButton(app.AppointmentId, options, Codes.AdminCancel, user.UserId));
                        }
                        return;
                    }
        ...
        {
            DataBase.Models.User admin = await context.Finder.FindAdminAsync();
            long chatId = e.CallbackQuery.Message.Chat.Id;
            DataBase.Models.User currentUser = await context.Finder.FindUserAsync(chatId);
            ...
            if (data.Error == "0" && !data.EmptyButton)
            {
                if (data.UserRole == UserRole.User)
                {
                    ...
                    {
                        List<Day> days = await context.Finder.FindDaysAsync(data.EntityId);
                    ...
                    {
                        Day day = await context.Finder.FindDayAsync(data.EntityId);
                        if (!day.IsWorkDay)
                        {
                        ...
                        List<Appointment> apps = await context.Finder.FindAppointmentsAsync(data.EntityId);
                    ...
                    {
                        Appointment app = await context.Finder.FindAppointmentAsync(data.EntityId);
                        if (!app.IsEmpty)
                        {
                ...
                if (data.UserRole == UserRole.Admin)
                {
                    if (data.Action == CallBackData.Action.Confirm)
                    {
                        if (data.Stage == Stage.Yes)
                        {
                            ...
                            app.IsConfirm = true;
                            await context.Updater.UpdateAppAsync(app);
                            var confirmedUser = await context.Finder.FindUserAsync(data.UserId);
                    ...
                    if (data.Action == CallBackData.Action.Add)
                    {
                        ...
                        {
                            List<Day> days = await context.Finder.FindDaysAsync(data.EntityId);
                            var user = await context.Finder.FindUserAsync(data.UserId);
                        ...
                        {
                            Appointment app = await context.Creater.AddAppointmentAsync(data.EntityId);
                    ...
                    if (data.Action == CallBackData.Action.Delete)
                    {
                        ...
                        if (data.Stage == Stage.Yes)
                        {
                            var app = await context.Finder.FindAppointmentAsync(data.EntityId);
                            var user = await context.Finder.FindUserAsync(data.UserId);
                            var day = await context.Finder.FindDayAsync(app.Day);
                    ...
                    if (data.Action == CallBackData.Action.WeekEnd)
                    {
                        ...
                        if (data.Stage == Stage.Day)
                        {
                            ...
                            {
                                day.IsWorkDay = true;
                                await context.Updater.UpdateDayAsync(day);
                            ...
                            List<Appointment> apps = await context.Finder.FindAppointmentsAsync(day.DayId);
                            bool AllEmpty = true;
                            foreach (Appointment app in apps)
                            {
                                if (!app.IsEmpty)
                                {
                                    foreach (Appointment appointment in apps)
                                    {
                                        ...
                                        {
                                            var user = await context.Finder.FindUserAsync(appointment.User);
                    ...
                    if (data.Action == CallBackData.Action.Mailing)
                    {
                        ...
                        if (data.Stage == Stage.Yes)
                        {
                            var users = await context.Finder.FindUsersAsync();
                            foreach (var user in users)
                            {
        ...
        public  void  BotStart()
        {
            ...
            Task.Run(() => StartNotificationAsync(DateTime.Now)); 
        }
        public  void BotStop()
        {
            bot.StopReceiving();
        ...
        public void SerializeBotConfig(StreamWriter writer)
        {
            ...
            serializer.Serialize(writer, this.botConfig);
        }
        public void SerializePersonalConfig(StreamWriter writer)
        {
            JsonSerializer serializer = new JsonSerializer();
            serializer.Serialize(writer, this.personalConfig);
        }
    }
}
